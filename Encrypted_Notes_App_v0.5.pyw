...
def separatewith():
    ...
    separatewindow.geometry("350x190")
    errorlabel = StringVar()
    errorlabel.set("")
    Label(separatewindow, text = "", bg='#bdbdbd').grid(row = 0)
    Label(separatewindow, text = "Separator: ", font = ("italic", 10), bg='#bdbdbd').grid(row = 1, column = 0, stick = E)
    Label(separatewindow, text = "Place the separator at the end of the note as well!\nThe separated note will have the current note's \nname and a number of the part at the end", bg='#bdbdbd').grid(row = 5, column= 1, stick = W)
    Label(separatewindow, text = "   ", bg='#bdbdbd').grid(row = 1, column = 2, stick = W)
    sep = Entry(separatewindow, bg='lightgrey')
    sep.grid(row = 1, column = 1, stick = W)
    sepbyline = IntVar()
    def checked():
        if sepbyline.get() == 1:
            sep['state'] = DISABLED
            ...
            sep['state'] = NORMAL
    Checkbutton(separatewindow, text = "Separate by line", variable = sepbyline, bg='lightgrey', command = checked).grid(row = 2, column = 1, stick = W)
    def close():
    ...
    def startseparation():
        if successfullydecrypted.get() == 1:
            ...
            for line in lines.splitlines():
                if sepbyline.get() == 1:
                    if os.path.isfile(nameofnote + str(i) + ".txt"):
                        try:
    ...
    separatenote = Button(separatewindow, text = "Separate the note", font = ("bold", 10), command = startseparation).grid(row = 3, column = 1, stick = W)
    Label(separatewindow, textvariable = errorlabel, fg = "red", bg='#bdbdbd').grid(row = 4, column = 1, stick = W)
...
def mergenotes():
    ...
    errorlabel = StringVar()
    errorlabel.set("")
    Label(mergewindow, text = "  ", bg='#bdbdbd').grid(row = 0, stick = W)
    Label(mergewindow, text = "Name of note to merge into: ", bg='#bdbdbd').grid(row = 1, column = 0, stick = W)
    notenametomergeinto = Entry(mergewindow, bg='lightgrey')
    notenametomergeinto.grid(row = 1, column = 1, stick = W)
    ...
    def startmerge():
        ...
        if ".txt" in notenametomergeinto.get():
            try:
                mainfile = open(notenametomergeinto.get(), "w+")
                openedmain = 1
            except:
                openedmain = 0
        ...
        for line in lines.splitlines():
            ...
            if ".txt" in line:
                ...
                if os.path.isfile(line):
                    try:
                        i += 1
                        tempfile = open(line, "r")
                        opened = 1
                        for line2 in lines2:
                            mainfile.write(line2)
                        tempfile.close()
                    except:
                        errorlabel.set("Cannot open file " + line)
                        cantopen.set((cantopen.get() + str(i) + " "))
                        opened = 0
                        errors += 1
                else:
                    i += 1
                    errorlabel.set("No such note as " + line)
                    filesdontexist.set((filesdontexist.get() + str(i) + " "))
                    errors += 1
            else:
                filename = line + ".txt"
                if os.path.isfile(line + ".txt"):
                    try:
                        ...
                        errorlabel.set("Cannot open file " + (line + ".txt"))
                        cantopen.set((cantopen.get() + str(i) + " "))
                        opened = 0
                        errors += 1
                else:
                    i += 1
                    errorlabel.set("No such note as " + (line+".txt"))
                    filesdontexist.set((filesdontexist.get() + str(i) + " "))
                    errors += 1
        if openedmain == 1 and opened == 1 and errors == 0:
            ...
            mainfile.close()
            failedwin()
    Button(mergewindow, text = "Merge", command = startmerge).grid(row = 3, column = 1, stick = W)
...
createnotebutton = Button(master, text = "Create/Save the note", command = lambda: areyousure(1)).grid(row = 1, column = 0, stick = E)
...
Label(master, text = "Note name: ", bg='#bdbdbd').grid (row = 0, column = 0, stick = E)
cleartextbox = Button(master, text = "               Clear               ", command = lambda: notetext.delete("1.0", "end")).grid(row = 4, column = 1, stick = E)
#separatebutton = Button(master, text = "Separate the note to \ndifferent multiple notes", command = separatewith).grid(row = 3, column = 0, stick = E)
